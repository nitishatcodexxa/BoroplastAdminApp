...
console.log(selected)
  function deleteAll(){
      ...
      fetch("http://localhost:5000/bulkDeleteUser", {
  ...
  handleRequestSort = (event, property) => {
   ...
   handleSelectAllClick = (event) => {
    if (event.target.checked) {
      const newSelected = this.state.userArray.map((n) => n.user_id);
...
let m = mm?mm.filter((e=>(e.username=="" || e.username.includes(this.state.search)))):null
...
<Button variant='contained' onClick={()=>this.props.navigate('/users/add')} disableElevation startIcon={<AddIcon />} size='small' sx={{textTransform:'none',backgroundColor:'#e26511'}}>
Add User
</Button>
   </Box>
  </Grid>
        ...
        <TableContainer sx={{minHeight:'50vh'}}>
          ...
          >
            <EnhancedTableHead
              ...
              rowCount={this.state.userArray.length}
            />
            <TableBody>
              {m.map((row, index) => {
                const isItemSelected = this.isSelected(row.user_id);
                ...
                return (
                  ...
                  color='primary'
                        ...
                        onClick={(event) => this.handleClick(event, row.user_id)}
                    ...
                    <TableCell align="center">411017</TableCell>
                 
                    <TableCell align="right"><BorderColorIcon sx={{color:'green',height:16,width:16}} onClick={this.edit.bind(this,row)}/>   <DeleteIcon sx={{color:'red',height:16,width:16, marginLeft:0.5,marginRight:0.5}} onClick={this.delete.bind(this,row.user_id)}/> <ContactPageIcon sx={{color:'#3f4290',height:16,width:16}}/>    </TableCell>
                  </TableRow>
                );
              })}
              {emptyRows > 0 && (
...
export default User
export function Userc(props){
  const navigate = useNavigate();
  const location = useLocation();
  return (<User location={location} navigate={navigate}></User>)
}