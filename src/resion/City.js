...
submit=()=>{
  if(this.state.state!=="" && this.state.state_id!=="" && this.state.city_name!=="" && this.state.city_id!==""){
 fetch("http://localhost:5000/updateCity", {
    headers:{
     // 'authorization': `Bearer ${localStorage.getItem('token')}`,
  'content-type':'application/json',
...
delete=(da)=>{
  fetch("http://localhost:5000/deleteCity", {
    headers:{
     // 'authorization': `Bearer ${localStorage.getItem('token')}`,
  'content-type':'application/json',
 // 'Access-Control-Allow-Origin': 'http://localhost:3000',
...
let m = mm?mm.filter((e=>(e.city_name=="" || e.city_name.includes(this.state.search)))):[]
        ...
        <TableBody>
          {m.map((row) => (
            ...
            >
              ...
              <TableCell align="center" sx={{fontSize:13,}}>{row.state}</TableCell>
